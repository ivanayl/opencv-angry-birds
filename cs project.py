...
class HelpMode(Mode):
    def appStarted(mode):
        mode.bkgd = mode.loadImage("help screen.jpg")
        mode.bkgd = mode.bkgd.resize((mode.width, mode.height), Image.ANTIALIAS)
...
class WinMode(Mode):
    def appStarted(mode):
        mode.bkgd = mode.loadImage("win screen.png")
        mode.bkgd = mode.bkgd.resize((mode.width, mode.height), Image.ANTIALIAS)
...
class LoseMode(Mode):
    def appStarted(mode):
        mode.bkgd = mode.loadImage("lose screen.jpg")
        mode.bkgd = mode.bkgd.resize((mode.width, mode.height), Image.ANTIALIAS)
...
class oneTierMode(Mode):
    def appStarted(mode):
        ...
        mode.ballx = None
        mode.bally = None
        mode.slingshot = mode.loadImage("slingshot.png")
        ...
        for i in range(woodRange):
            ...
            if i%2 == 0:
                if yes == 0:
                    wood = Wood(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodShift*3/2, 1, "small wood.png", [])
                    mode.wood.append(wood)
                else:
                    metal = Metal(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodShift*3/2, 1, "metal block.png", [])
                    metal.h = mode.woodSpacing
                    metal.w = mode.woodShift
                    mode.wood.append(metal)
            elif i%2 != 0:
                if yes == 0:
                    flat = flatWood(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodSpacing, 1, "flat wood.png", [])
                    mode.wood.append(flat)
                else:
                    metal = flatMetal(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodSpacing, 1, "flat metal.png", [])
        ...
        for j in range(numPigs):
            ...
            pig = Pig(mode, pigX + space, mode.height - mode.marginY - mode.woodShift*3/2 - mode.woodSpacing, 1, "piggy.png", [])
            for wood in mode.wood:
    ...
    def redrawAll(mode, canvas):
        ...
        if mode.app.video:
            frame = mode.loadImage("frame.jpg")
    ...
    def appStarted(mode):
        ...
        for tier in range(2):
            ...
            for i in range(woodRange):
                ...
                if i%2 == 0:
                    if yes == 0:
                        wood = Wood(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodShift*3/2 - tierShift, tier + 1, "small wood.png", [])
                        mode.wood.append(wood)
                    else:
                        metal = Metal(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodShift*3/2 - tierShift, tier + 1, "metal block.png", [])
                        metal.h = mode.woodSpacing
                        metal.w = mode.woodShift
                        mode.wood.append(metal)
                elif i%2 != 0:
                    if yes == 0:
                        flat = flatWood(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodSpacing - tierShift, tier + 1, "flat wood.png", [])
                        mode.wood.append(flat)
                    else:
                        metal = flatMetal(mode, startX + mode.woodSpacing*(i/2), mode.height - mode.marginY - mode.woodSpacing - tierShift, tier + 1, "flat metal.png", [])
                        metal.w = mode.woodSpacing
                        metal.h = mode.woodShift
                        mode.wood.append(metal)
            #pigs
            ...
            for j in range(numPigs):
                space = random.randint(pigWidth//2, pigWidth)
                pig = Pig(mode, pigX + space, mode.height - mode.marginY - mode.woodShift*3/2 - mode.woodSpacing - tierShift, tier + 1, "piggy.png", [])
                for wood in mode.wood:
                    if type(wood) == flatWood or type(wood) == flatMetal:
